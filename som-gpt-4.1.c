...
static void som_initialize_reloc_queue(struct reloc_queue *queue) {
    if (!queue) return;
    for (int i = 0; i < 4; ++i) {
        queue[i].reloc = NULL;
        queue[i].size = 0;
...
static void som_reloc_queue_insert(unsigned char *p, unsigned int size, struct reloc_queue *queue) {
    if (!p || !queue) {
        return;
    }
    for (int i = 3; i > 0; --i) {
...
static void som_reloc_queue_fix(struct reloc_queue *queue, unsigned int idx)
{
    ...
    switch (idx) {
        case 0:
            return;
        case 1: {
            struct reloc_queue temp = queue[0];
            queue[0] = queue[1];
            queue[1] = temp;
            return;
        }
...
static int som_reloc_queue_find(unsigned char *p, unsigned int size, struct reloc_queue *queue) {
    if (!p || !queue)
        return -1;
    for (int i = 0; i < 4; ++i) {
        if (queue[i].reloc && queue[i].size == size && memcmp(p, queue[i].reloc, size) == 0) {
            return i;
        }
    }
    return -1;
               ...
               struct reloc_queue *queue)
{
    ...
    queue_index = som_reloc_queue_find(p, size, queue);

    if (queue_index >= 0) {
        if (*subspace_reloc_sizep != UINT_MAX) {
            bfd_put_8(abfd, (unsigned char)(R_PREV_FIXUP + queue_index), p);
            p += 1;
        ...
        if (UINT_MAX - *subspace_reloc_sizep < size)
            return NULL;
               ...
               struct reloc_queue *queue)
{
    ...
    if (skip > 0)
    {
        ...
        {
            unsigned int diff = (skip >> 2) - 1;

            if (skip <= 0x60)
            {
                bfd_put_8(abfd, R_NO_RELOCATION + diff, p);
                *subspace_reloc_sizep += 1;
                p++;
            }
            else if (skip <= 0x1000)
            {
                bfd_put_8(abfd, R_NO_RELOCATION + 24 + (diff >> 8), p);
                bfd_put_8(abfd, diff & 0xff, p + 1);
                p = try_prev_fixup(abfd, subspace_reloc_sizep, p, 2, queue);
            }
            else
            {
                bfd_put_8(abfd, R_NO_RELOCATION + 28 + (diff >> 16), p);
                bfd_put_16(abfd, (bfd_vma)(diff & 0xffff), p + 1);
        ...
        {
            bfd_put_8(abfd, R_NO_RELOCATION + 31, p);
            bfd_put_8(abfd, ((skip - 1) >> 16) & 0xff, p + 1);
            bfd_put_16(abfd, (bfd_vma)((skip - 1) & 0xffff), p + 2);
            p = try_prev_fixup(abfd, subspace_reloc_sizep, p, 4, queue);
        }
    }
                 ...
                 struct reloc_queue *queue)
{
    ...
    bfd_vma adj_addend = addend;
    unsigned int size = 0;

    if (adj_addend + 0x80 < 0x100) {
        bfd_put_8(abfd, R_DATA_OVERRIDE + 1, p);
        bfd_put_8(abfd, adj_addend, p + 1);
        size = 2;
    } else if (adj_addend + 0x8000 < 0x10000) {
        bfd_put_8(abfd, R_DATA_OVERRIDE + 2, p);
        bfd_put_16(abfd, adj_addend, p + 1);
        size = 3;
    } else if (adj_addend + 0x800000 < 0x1000000) {
        bfd_put_8(abfd, R_DATA_OVERRIDE + 3, p);
        bfd_put_8(abfd, adj_addend >> 16, p + 1);
        bfd_put_16(abfd, adj_addend, p + 2);
        size = 4;
    } else {
        bfd_put_8(abfd, R_DATA_OVERRIDE + 4, p);
        bfd_put_32(abfd, adj_addend, p + 1);
        size = 5;
    }

    return try_prev_fixup(abfd, subspace_reloc_sizep, p, size, queue);
}
               ...
               struct reloc_queue *queue)
{
    ...
    if (sym_num < 0x100)
    {
        ...
        switch (arg_bits)
        {
            ...
            case (base | bits[0] | 1):
                type = 2;
                break;
            case (base | bits[0] | bits[1]):
            case (base | bits[0] | bits[1] | 1):
                type = 3;
                break;
            case (base | bits[0] | bits[1] | bits[2]):
            case (base | bits[0] | bits[1] | bits[2] | 1):
                type = 4;
                break;
            default:
                type = -1;
    ...
    if (!done)
    {
        int temp_type = rtn_bits;
        unsigned int shifted6 = (arg_bits >> 6) & 0xf;
        unsigned int shifted8 = (arg_bits >> 8) & 3;
        unsigned int shifted6b = (arg_bits >> 6) & 3;
        unsigned int shifted2 = (arg_bits >> 2) & 0xf;
        unsigned int shifted4 = (arg_bits >> 4) & 3;
        unsigned int shifted2b = (arg_bits >> 2) & 3;

        if (shifted6 == 0xe)
            temp_type += 9 * 40;
        else
            temp_type += (3 * shifted8 + shifted6b) * 40;
        if (shifted2 == 0xe)
            temp_type += 9 * 4;
        else
            temp_type += (3 * shifted4 + shifted2b) * 4;

        bfd_put_8(abfd,
                  bfd_reloc->howto->type + 10
                  + 2 * (sym_num >= 0x100)
                  + (temp_type >= 0x100),
                  p);
        bfd_put_8(abfd, temp_type, p + 1);
...
static bfd_reloc_status_type hppa_som_reloc(
    ...
    char **error_message)
{
    if (reloc_entry && input_section && output_bfd) {
...
{
    int **final_types = NULL;
    int *final_type = NULL;
    size_t max_types = 6;
    size_t amt = sizeof(int);
    size_t type_ptr_amt = sizeof(int *) * max_types;
    size_t i;

    final_types = bfd_alloc(abfd, type_ptr_amt);
    ...
    switch (field) {
        ...
        case e_ltsel:
        case e_rtsel: {
            int *mode_type = bfd_alloc(abfd, amt);
            if (!mode_type)
                return NULL;
            if (field == e_tsel)
                *mode_type = R_FSEL;
        ...
        case e_lssel:
        case e_rssel: {
            int *s_mode = bfd_alloc(abfd, amt);
            if (!s_mode)
                return NULL;
            *s_mode = R_S_MODE;
            final_types[0] = s_mode;
            final_types[1] = final_type;
            *final_type = base_type;
            break;
        }
        ...
        case e_lsel:
        case e_rsel: {
            int *n_mode = bfd_alloc(abfd, amt);
            if (!n_mode)
                return NULL;
            *n_mode = R_N_MODE;
            final_types[0] = n_mode;
            final_types[1] = final_type;
            *final_type = base_type;
            break;
        }
        ...
        case e_ldsel:
        case e_rdsel: {
            int *d_mode = bfd_alloc(abfd, amt);
            if (!d_mode)
                return NULL;
            *d_mode = R_D_MODE;
            final_types[0] = d_mode;
            final_types[1] = final_type;
            *final_type = base_type;
            break;
        }
        ...
        case e_lrsel:
        case e_rrsel: {
            int *r_mode = bfd_alloc(abfd, amt);
            if (!r_mode)
                return NULL;
            *r_mode = R_R_MODE;
            final_types[0] = r_mode;
            final_types[1] = final_type;
            *final_type = base_type;
            break;
        }
        ...
        case e_nlrsel: {
            ...
            *n0sel = R_N0SEL;
            ...
            mode = bfd_alloc(abfd, amt);
            if (!mode)
                return NULL;
            *mode = (field == e_nlsel) ? R_N_MODE : R_R_MODE;
            final_types[1] = mode;
    ...
    switch (base_type) {
        case R_HPPA:
            if (sym_diff) {
                ...
                *final_type = (format == 32) ? R_DATA_EXPR : R_CODE_EXPR;
                break;
        ...
        case R_HPPA_PCREL_CALL: {
            ...
            int *pcrel_mode = bfd_alloc(abfd, amt);
            if (!pcrel_mode)
                return NULL;
            *pcrel_mode = (format == 17) ? R_SHORT_PCREL_MODE : R_LONG_PCREL_MODE;
            final_types[0] = pcrel_mode;
            final_types[1] = final_type;
            *final_type = base_type;
    #endif
            break;
...
static reloc_howto_type *som_bfd_reloc_type_lookup(bfd *abfd ATTRIBUTE_UNUSED, bfd_reloc_code_real_type code) {
  int idx = (int)code;
  if (idx >= (int)R_NO_RELOCATION && idx < (int)R_NO_RELOCATION + 255) {
    if ((int)som_hppa_howto_table[idx].type != idx) {
      return NULL;
    }
    return &som_hppa_howto_table[idx];
  }
  return NULL;
}

static reloc_howto_type *som_bfd_reloc_name_lookup(bfd *abfd ATTRIBUTE_UNUSED, const char *r_name) {
    size_t count = sizeof(som_hppa_howto_table) / sizeof(som_hppa_howto_table[0]);
    if (!r_name) {
        return NULL;
    }
    for (size_t i = 0; i < count; i++) {
        const char *name = som_hppa_howto_table[i].name;
...
static void som_swap_clock_out(const struct som_clock *src, struct som_external_clock *dst) {
    if (!src || !dst) {
        return;
    }
...
static int
som_swap_header_in(const struct som_external_header *src, struct som_header *dst)
{
    if (!src || !dst) {
        return -1;
    }
    ...
    if (som_swap_clock_in(&src->file_time, &dst->file_time) != 0) {
        return -1;
    }
...
static void som_swap_header_out(const struct som_header *src, struct som_external_header *dst) {
    if (!src || !dst) {
        return;
    }
...
static void som_swap_space_dictionary_in(const struct som_external_space_dictionary_record *src,
                                         struct som_space_dictionary_record *dst)
{
    ...
    dst->is_loadable = (flags & SOM_SPACE_IS_LOADABLE) ? 1 : 0;
    dst->is_defined = (flags & SOM_SPACE_IS_DEFINED) ? 1 : 0;
    dst->is_private = (flags & SOM_SPACE_IS_PRIVATE) ? 1 : 0;
    dst->has_intermediate_code = (flags & SOM_SPACE_HAS_INTERMEDIATE_CODE) ? 1 : 0;
    dst->is_tspecific = (flags & SOM_SPACE_IS_TSPECIFIC) ? 1 : 0;
    dst->reserved = 0;
...
static void som_swap_space_dictionary_out(const struct som_space_dictionary_record *src,
...
static void som_swap_subspace_dictionary_in(
    const struct som_external_subspace_dictionary_record *src,
    struct som_subspace_dictionary_record *dst)
{
    ...
    dst->memory_resident = !!(flags & SOM_SUBSPACE_MEMORY_RESIDENT);
    dst->dup_common = !!(flags & SOM_SUBSPACE_DUP_COMMON);
    dst->is_common = !!(flags & SOM_SUBSPACE_IS_COMMON);
    dst->is_loadable = !!(flags & SOM_SUBSPACE_IS_LOADABLE);
    dst->quadrant = (flags >> SOM_SUBSPACE_QUADRANT_SH) & SOM_SUBSPACE_QUADRANT_MASK;
    dst->initially_frozen = !!(flags & SOM_SUBSPACE_INITIALLY_FROZEN);
    dst->is_first = !!(flags & SOM_SUBSPACE_IS_FIRST);
    dst->code_only = !!(flags & SOM_SUBSPACE_CODE_ONLY);
    dst->sort_key = (flags >> SOM_SUBSPACE_SORT_KEY_SH) & SOM_SUBSPACE_SORT_KEY_MASK;
    dst->replicate_init = !!(flags & SOM_SUBSPACE_REPLICATE_INIT);
    dst->continuation = !!(flags & SOM_SUBSPACE_CONTINUATION);
    dst->is_tspecific = !!(flags & SOM_SUBSPACE_IS_TSPECIFIC);
    dst->is_comdat = !!(flags & SOM_SUBSPACE_IS_COMDAT);
    dst->reserved = 0;
...
static void som_swap_subspace_dictionary_record_out(
    const struct som_subspace_dictionary_record *src,
    struct som_external_subspace_dictionary_record *dst)
{
    ...
    if (src->memory_resident)   flags |= SOM_SUBSPACE_MEMORY_RESIDENT;
    if (src->dup_common)        flags |= SOM_SUBSPACE_DUP_COMMON;
    if (src->is_common)         flags |= SOM_SUBSPACE_IS_COMMON;
    if (src->is_loadable)       flags |= SOM_SUBSPACE_IS_LOADABLE;
    if (src->initially_frozen)  flags |= SOM_SUBSPACE_INITIALLY_FROZEN;
    if (src->is_first)          flags |= SOM_SUBSPACE_IS_FIRST;
    if (src->code_only)         flags |= SOM_SUBSPACE_CODE_ONLY;
    if (src->replicate_init)    flags |= SOM_SUBSPACE_REPLICATE_INIT;
    if (src->continuation)      flags |= SOM_SUBSPACE_CONTINUATION;
    if (src->is_tspecific)      flags |= SOM_SUBSPACE_IS_TSPECIFIC;
    if (src->is_comdat)         flags |= SOM_SUBSPACE_IS_COMDAT;
...
static void som_swap_aux_id_in(const struct som_external_aux_id *src, struct som_aux_id *dst) {
    if (!src || !dst) {
        return;
    }
    unsigned int flags = bfd_getb32(src->flags);

    dst->mandatory = (flags & SOM_AUX_ID_MANDATORY) ? 1 : 0;
    dst->copy = (flags & SOM_AUX_ID_COPY) ? 1 : 0;
    dst->append = (flags & SOM_AUX_ID_APPEND) ? 1 : 0;
    dst->ignore = (flags & SOM_AUX_ID_IGNORE) ? 1 : 0;
    dst->type = (flags >> SOM_AUX_ID_TYPE_SH) & SOM_AUX_ID_TYPE_MASK;
    dst->length = bfd_getb32(src->length);
}

static void som_swap_aux_id_out(const struct som_aux_id *src, struct som_external_aux_id *dst)
{
    if (!src || !dst) {
        return;
    }
    ...
    if (src->mandatory)  flags |= SOM_AUX_ID_MANDATORY;
    if (src->copy)       flags |= SOM_AUX_ID_COPY;
    if (src->append)     flags |= SOM_AUX_ID_APPEND;
    if (src->ignore)     flags |= SOM_AUX_ID_IGNORE;
...
static void som_swap_string_auxhdr_out(const struct som_string_auxhdr *src,
...
static void som_swap_compilation_unit_out(const struct som_compilation_unit *src, struct som_external_compilation_unit *dst)
{
    if (!src || !dst)
        return;
...
static void som_swap_exec_auxhdr_in(const struct som_external_exec_auxhdr *src, struct som_exec_auxhdr *dst) {
    if (!src || !dst) {
        return;
    }
...
static void som_swap_exec_auxhdr_out(const struct som_exec_auxhdr *src, struct som_external_exec_auxhdr *dst)
{
    if (!src || !dst) {
        return;
    }
...
static int validate_lst_header(const struct som_external_lst_header *src, struct som_lst_header *dst) {
    if (!src || !dst) {
        return 0;
    }
    return 1;
...
static void som_swap_lst_header_in(const struct som_external_lst_header *src, struct som_lst_header *dst) {
    if (!validate_lst_header(src, dst)) {
        return;
    }
    dst->system_id    = bfd_getb16(src->system_id);
    dst->a_magic      = bfd_getb16(src->a_magic);
...
static bfd_cleanup som_object_setup(bfd *abfd,
                                   ...
                                   unsigned long current_offset)
{
  ...
  if (aux_hdrp)
  {
    ...
    if ((aux_hdrp->exec_entry == 0 && !(abfd->flags & DYNAMIC)) ||
        (aux_hdrp->exec_entry & 0x3) != 0 || !found)
    {
...
setup_sections(bfd *abfd, struct som_header *file_hdr, unsigned long current_offset)
{
    ...
    unsigned int space_index, i, total_subspaces = 0;
    asection *section;
    size_t amt;
    ...
    if (bfd_seek(abfd, current_offset + file_hdr->space_strings_location, SEEK_SET) != 0)
        goto error_return;
    space_strings = _bfd_malloc_and_read(abfd, amt + 1, amt);
    if (!space_strings)
        goto error_return;
    space_strings[amt] = 0;

    for (space_index = 0; space_index < file_hdr->space_total; ++space_index) {
        struct som_som_space_dictionary_record space;
        ...
        if (bfd_seek(abfd, current_offset + file_hdr->subspace_location + space.subspace_index * sizeof ext_subspace, SEEK_SET) != 0)
            goto error_return;
        ...
        for (subspace_index = 0; subspace_index < space.subspace_quantity; ++subspace_index) {
            ...
            if (!bfd_som_set_subsection_attributes(subspace_asect, space_asect, subspace.access_control_bits,
            ...
            switch (subspace.access_control_bits >> 4) {
                ...
                case 0x1:
                    ...
                    break;
                case 0x2: case 0x4: case 0x5: case 0x6: case 0x7:
...
static bool som_mkobject(bfd *abfd)
{
...
static bool som_prep_headers(bfd *abfd)
{
    ...
    for (section = abfd->sections; section; section = section->next) {
        ...
        if (som_is_space(section)) {
            ...
            if (!som_section_data(section)->space_dict) {
                return false;
            }
            som_section_data(section)->space_dict->loader_fix_index = -1;
            som_section_data(section)->space_dict->init_pointer_index = -1;
            if (som_section_data(section)->copy_data) {
            ...
            if (!som_section_data(section)->subspace_dict) {
                return false;
            }
            ...
            som_section_data(section)->subspace_dict->alignment = (1U << section->alignment_power);
            if (som_section_data(section)->copy_data) {
...
static bool som_is_space(asection *section) {
    struct som_sec_data *sec_data = som_section_data(section);
    struct som_copy_data *copy_data;
    if (!sec_data || !(copy_data = sec_data->copy_data))
        return false;
...
{
    som_section_data_type *data;
    if (section == NULL)
        return false;
    data = som_section_data(section);
    if (data == NULL || data->copy_data == NULL)
        return false;
    if (data->copy_data->container == section ||
        (data->copy_data->container &&
...
static bool som_is_container(asection *space, asection *subspace) {
    ...
    if (!subspace_data || !subspace_data->copy_data || !subspace_data->copy_data->container) {
        return false;
    }
    if (subspace_data->copy_data->container == space) {
        return true;
    }
    if (subspace_data->copy_data->container->output_section == space) {
        return true;
    }
    return false;
}
...
static int compare_syms(const void *arg1, const void *arg2) {
    ...
    count1 = (sym1->flags & BSF_SECTION_SYM) ? sym1->udata.i : som_symbol_data(sym1)->reloc_count;
    count2 = (sym2->flags & BSF_SECTION_SYM) ? sym2->udata.i : som_symbol_data(sym2)->reloc_count;
...
{
    const asection *subspace1 = *(const asection * const *)arg1;
    const asection *subspace2 = *(const asection * const *)arg2;

    if (subspace1 == NULL || subspace2 == NULL)
        return 0;
...
static bool som_prep_for_fixups(bfd *abfd, asymbol **syms, unsigned long num_syms)
{
    ...
    for (i = 0; i < num_syms; ++i) {
        void *sym_data = som_symbol_data(syms[i]);
        if (sym_data == NULL || (syms[i]->flags & BSF_SECTION_SYM)) {
            syms[i]->flags |= BSF_SECTION_SYM;
            syms[i]->udata.i = 0;
        } else {
            ((som_symbol_type *)sym_data)->reloc_count = 0;
        }
    }

    for (section = abfd->sections; section != NULL; section = section->next) {
        long rcnt = section->reloc_count;
        if (rcnt <= 0 || section->orelocation == NULL)
            continue;

        for (long j = 1; j < rcnt; ++j) {
            arelent *reloc = section->orelocation[j];
            int scale;

            if (reloc == NULL || reloc->sym_ptr_ptr == NULL)
                continue;

            asymbol *reloc_sym = *reloc->sym_ptr_ptr;
            if (reloc_sym == NULL || bfd_is_abs_section(reloc_sym->section))
                continue;

            scale = (reloc->howto->type == R_DP_RELATIVE || reloc->howto->type == R_CODE_ONE_SYMBOL) ? 2 : 1;

            if (reloc_sym->flags & BSF_SECTION_SYM) {
                reloc_sym->udata.i += scale;
            } else {
                void *sym_data = som_symbol_data(reloc_sym);
                if (sym_data != NULL)
                    ((som_symbol_type *)sym_data)->reloc_count += scale;
            }
        }
    }
    ...
    for (i = 0; i < num_syms; ++i) {
        if (sorted_syms[i]->flags & BSF_SECTION_SYM) {
            sorted_syms[i]->udata.i = (int)i;
        } else {
            void *sym_data = som_symbol_data(sorted_syms[i]);
            if (sym_data != NULL)
                ((som_symbol_type *)sym_data)->index = (int)i;
        }
    }
    return true;
}
...
static bool
som_write_fixups(bfd *abfd, unsigned long current_offset, unsigned int *total_reloc_sizep)
{
    unsigned int i, j, num_spaces, total_reloc_size = 0;
    ...
    for (i = 0; i < num_spaces; i++) {
        ...
        while (section && !som_is_space(section)) section = section->next;
        if (!section) break;

        for (subsection = abfd->sections; subsection != NULL; subsection = subsection->next) {
            unsigned int reloc_offset = 0, current_rounding_mode = R_N_MODE;
            ...
            for (j = 0; j < subsection->reloc_count; j++) {
                ...
                if (bfd_reloc->address < reloc_offset) {
                    _bfd_error_handler(
                        ...
                        abfd, subsection, (uint64_t) bfd_reloc->address, bfd_reloc->howto->name);
                    bfd_set_error(bfd_error_bad_value);
                    return false;
                }

                if (!bfd_reloc_offset_in_range(bfd_reloc->howto, abfd, subsection, bfd_reloc->address)) {
                    _bfd_error_handler(
                ...
                sym_num = ((*bfd_reloc->sym_ptr_ptr)->flags & BSF_SECTION_SYM)
                          ? (*bfd_reloc->sym_ptr_ptr)->udata.i
                          : som_symbol_data(*bfd_reloc->sym_ptr_ptr)->index;

                if ((size_t)(p - tmp_space) + 512 > sizeof(tmp_space)) {
                    amt = p - tmp_space;
           ...
           unsigned int *strings_size, unsigned int *strx)
{
    ...
    size_t needed = (4 + total_len + 3) & ~((size_t)3);

    size_t offset = (size_t)(p - *buf);

    if (offset + needed > *buflen) {
        if (bfd_write(*buf, offset, abfd) != offset)
            return NULL;

        if (needed > *buflen) {
            size_t new_len = (*buflen * 2 < needed) ? needed : (*buflen * 2);
            char *new_buf = (char *)bfd_malloc(new_len);
            if (!new_buf)
                return NULL;
            free(*buf);
            *buf = new_buf;
    ...
    *strx = *strings_size;
    ...
    p += str_len;
    *p++ = '\0';
    *strings_size += str_len + 1;

    size_t align = ((size_t)(p - *buf)) & 3;
    if (align) {
        align = 4 - align;
        memset(p, 0, align);
        p += align;
...
static bool som_write_space_strings(bfd *abfd, unsigned long current_offset, unsigned int *strings_size)
{
    ...
    *strings_size = 0;
    ...
    size_t amt = (size_t)(p - tmp_space);
    bool ok = true;
    if (amt != 0)
        ok = bfd_write(tmp_space, amt, abfd) == amt;
...
static bool som_write_symbol_strings(bfd *abfd,
                                     unsigned long current_offset,
                                     asymbol **syms,
                                     ...
                                     struct som_compilation_unit *compilation_unit)
{
    ...
    *strings_size = 0;
    ...
    if (compilation_unit) {
        ...
        for (i = 0; i < 4; i++) {
            p = add_string(p, names[i]->name, abfd, &tmp_space, &tmp_space_size, strings_size, &names[i]->strx);
            if (!p) {
                free(tmp_space);
                return false;
            }
    ...
    size_t amt = (size_t)(p - tmp_space);
    ok = amt ? (bfd_write(tmp_space, amt, abfd) == amt) : true;
    free(tmp_space);
    return ok;
}
...
static bool som_begin_writing(bfd *abfd)
{
    ...
    if (obj_som_version_hdr(abfd) != NULL) {
        ...
        len = obj_som_version_hdr(abfd)->header_id.length;
        if (len < 4)
            return false;
        len -= 4;
        obj_som_file_hdr(abfd)->aux_header_size += len;
    ...
    if (obj_som_copyright_hdr(abfd) != NULL) {
        ...
        len = obj_som_copyright_hdr(abfd)->header_id.length;
        if (len < 4)
            return false;
        len -= 4;
        obj_som_file_hdr(abfd)->aux_header_size += len;
    ...
    for (i = 0; i < num_spaces; i++) {
        ...
        for (subsection = abfd->sections; subsection != NULL; subsection = subsection->next) {
            ...
            if (first_subspace && (abfd->flags & (EXEC_P | DYNAMIC))) {
                ...
                unsigned long vma_offset = subsection->vma - subspace_offset;
                current_offset += vma_offset;
                if (exec_header) {
                    if (subsection->flags & SEC_CODE)
                        exec_header->exec_tsize += vma_offset;
                    else
                        exec_header->exec_dsize += vma_offset;
                }
                subspace_offset += vma_offset;
            }
            ...
            if (subsection->flags & SEC_LOAD) {
                ...
                if (som_section_data(subsection)) {
                ...
                if (exec_header && (abfd->flags & (EXEC_P | DYNAMIC)))
...
som_finish_writing(bfd *abfd)
{
    ...
    // -------- Second Pass: Unloadable Subspaces --------
    section = abfd->sections;
    for (i = 0; i < num_spaces; i++)
    {
        asection *subsection;
    ...
    // -------- Write Space Dictionary Records --------
...
static uint32_t som_compute_checksum(const struct som_external_header *hdr) {
    if (!hdr) {
        return 0;
    }
    size_t count = sizeof(*hdr) / sizeof(uint32_t);
    const uint32_t *buffer = (const uint32_t *)hdr;
    uint32_t checksum = 0;
    for (size_t i = 0; i < count; i++) {
        checksum ^= buffer[i];
    }
...
static void som_bfd_derive_misc_symbol_info(bfd *abfd ATTRIBUTE_UNUSED, asymbol *sym, struct som_misc_symbol_info *info) {
    ...
    {
        struct som_section_data_struct *ssd = som_section_data(sym->section);
        if (ssd && ssd->subspace_dict &&
            info->symbol_scope == SS_UNIVERSAL &&
            (info->symbol_type == ST_ENTRY ||
             info->symbol_type == ST_CODE ||
             info->symbol_type == ST_DATA)) {
            info->is_comdat = ssd->subspace_dict->is_comdat;
            info->is_common = ssd->subspace_dict->is_common;
            info->dup_common = ssd->subspace_dict->dup_common;
        }
    }
}
...
static bool som_build_and_write_symbol_table(bfd *abfd)
{
    ...
    if (_bfd_mul_overflow(num_syms, sizeof(struct som_external_symbol_dictionary_record), &amt)) {
        bfd_set_error(bfd_error_no_memory);
        return false;
    }
    if (num_syms == 0) {
    ...
    symtab_size = (bfd_size_type)num_syms * sizeof(struct som_external_symbol_dictionary_record);
...
static bool som_write_object_contents(bfd *abfd)
{
    if (abfd == NULL)
        return false;
...
static long som_get_symtab_upper_bound(bfd *abfd)
{
    ...
    return (symcount + 1) * (long)sizeof(asymbol *);
}
...
asection *bfd_section_from_som_symbol(bfd *abfd, struct som_external_symbol_dictionary_record *symbol) {
    ...
    int is_executable = (abfd->flags & (EXEC_P | DYNAMIC)) != 0;
    int is_func_symbol = (symbol_type == ST_ENTRY || symbol_type == ST_PRI_PROG || symbol_type == ST_SEC_PROG || symbol_type == ST_MILLICODE);

    if (!is_executable || !is_func_symbol) {
        int idx = (bfd_getb32(symbol->info) >> SOM_SYMBOL_SYMBOL_INFO_SH) & SOM_SYMBOL_SYMBOL_INFO_MASK;
        for (asection *section = abfd->sections; section != NULL; section = section->next) {
        ...
        for (asection *section = abfd->sections; section != NULL; section = section->next) {
            if (value >= section->vma && value <= section->vma + section->size && som_is_subspace(section)) {
                return section;
            }
        }
    }
...
static unsigned int som_slurp_symbol_table(bfd *abfd)
{
    ...
    rc = bfd_seek(abfd, obj_som_sym_filepos(abfd), SEEK_SET);
    if (rc != 0)
        goto error_return;
    buf = (struct som_external_symbol_dictionary_record *)_bfd_malloc_and_read(abfd, amt, amt);
    if (buf == NULL)
        goto error_return;
    ...
    struct som_external_symbol_dictionary_record *bufp = buf;
    struct som_external_symbol_dictionary_record *endbufp = buf + symbol_count;
    sym = symbase;

    for (; bufp < endbufp; ++bufp) {
        ...
        switch (symbol_type) {
            case ST_NULL:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_UNKNOWN; break;
            case ST_ABSOLUTE:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_ABSOLUTE; break;
            case ST_DATA:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_DATA; break;
            case ST_CODE:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_CODE; break;
            case ST_PRI_PROG:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_PRI_PROG; break;
            case ST_SEC_PROG:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_SEC_PROG; break;
            case ST_ENTRY:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_ENTRY; break;
            case ST_MILLICODE:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_MILLICODE; break;
            case ST_PLABEL:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_PLABEL; break;
            default:
                som_symbol_data(sym)->som_type = SYMBOL_TYPE_UNKNOWN; break;
        }
        ...
        switch (symbol_type) {
            ...
            case ST_MILLICODE:
                ...
                som_symbol_data(sym)->tc_data.ap.hppa_priv_level = sym->symbol.value & 0x3;
        ...
        if (sym->symbol.name && sym->symbol.section && 
            sym->symbol.name[0] == '$' &&
            sym->symbol.name[strlen(sym->symbol.name) - 1] == '$' &&
            strcmp(sym->symbol.name, sym->symbol.section->name) == 0)
        {
            sym->symbol.flags |= BSF_SECTION_SYM;
        }
        else if (sym->symbol.name &&
                 startswith(sym->symbol.name, "L$0\002"))
        {
            sym->symbol.flags |= BSF_SECTION_SYM;
            sym->symbol.name = sym->symbol.section ? sym->symbol.section->name : sym->symbol.name;
        }
        else if (sym->symbol.name &&
                 startswith(sym->symbol.name, "L$0\001"))
        {
    ...
    abfd->symcount = (unsigned int)(sym - symbase);
...
static void som_print_symbol(bfd *abfd, void *afile, asymbol *symbol, bfd_print_symbol_type how)
{
    ...
    switch (how)
    {
        ...
        {
            const char *section_name = (symbol->section && symbol->section->name)
                                       ? symbol->section->name
                                       : "(*none*)";
            bfd_print_symbol_vandf(abfd, file, symbol);
            fprintf(file, " %s\t%s", section_name, symbol->name ? symbol->name : "(null)");
            break;
        }
...
static bool som_bfd_is_local_label_name(const bfd *abfd ATTRIBUTE_UNUSED, const char *name)
{
                   ...
                   bool just_count)
{
    ...
    int variables[26] = {0};
    int stack[20] = {0};
    int count = 0, prev_fixup = 0, saved_unwind_bits = 0;
    int *sp = stack;
        ...
        while (*cp) {
            ...
            switch (varname) {
                case 'L':
                    ...
                    break;
                case 'S':
                    if (!just_count && symbols && (unsigned int)c < symcount)
        ...
        if (som_hppa_howto_table[op].type != R_DATA_OVERRIDE &&
            ...
            if (!just_count) {
                if (som_hppa_howto_table[op].type == R_ENTRY)
                    ...
                    rptr->addend = var('V');
                    if (rptr->addend == 0 && (section->flags & SEC_HAS_CONTENTS)) {
                        if (!section->contents) {
                        ...
                        if (offset >= var('L') && (offset - var('L')) <= section->size &&
                            section->size - (offset - var('L')) >= 4) {
                            rptr->addend = bfd_get_32(section->owner,
                                                      section->contents + offset - var('L'));
                        }
                    }
                } else {
                    rptr->addend = var('V');
...
static bool som_slurp_reloc_table(bfd *abfd, asection *section, asymbol **symbols, bool just_count)
{
    ...
    if (section->reloc_count == (unsigned)-1)
    {
        ...
        section->reloc_count = som_set_reloc_info(
            external_relocs, fixup_stream_size, NULL, NULL, NULL, 0, true);
    ...
    som_set_reloc_info(
        external_relocs, fixup_stream_size,
...
static long som_get_reloc_upper_bound(bfd *abfd, sec_ptr asect)
{
    ...
    if (asect->flags & SEC_RELOC) {
        ...
        if (asect->reloc_count < 0)
            return -1;
        return ((long)asect->reloc_count + 1) * (long)sizeof(arelent *);
    }
...
static long som_canonicalize_reloc(bfd *abfd, sec_ptr section, arelent **relptr, asymbol **symbols) {
    ...
    if (!section || !section->relocation || !relptr) {
        return -1;
    }
...
static bool som_new_section_hook(bfd *abfd, asection *newsect)
{
    if (abfd == NULL || newsect == NULL)
        return false;
...
static bool som_bfd_copy_private_symbol_data(bfd *ibfd, asymbol *isymbol, bfd *obfd, asymbol *osymbol)
{
    if (!ibfd || !obfd || !isymbol || !osymbol)
        return false;
    ...
    output_symbol->tc_data.ap.hppa_arg_reloc = input_symbol->tc_data.ap.hppa_arg_reloc;
...
static bool som_bfd_copy_private_section_data(bfd *ibfd,
                                             asection *isection,
                                             bfd *obfd,
                                             asection *osection,
                                             struct bfd_link_info *link_info)
{
    ...
    struct som_copyable_section_data_struct *src_data = som_section_data(isection)->copy_data;
    struct som_copyable_section_data_struct *dst_data;

    if (!src_data)
        return true;

    dst_data = bfd_zalloc(obfd, amt);
    if (!dst_data)
        return false;

    memcpy(dst_data, src_data, amt);
    som_section_data(osection)->copy_data = dst_data;

    if (dst_data->container) {
        if (dst_data->container->output_section) {
            dst_data->container = dst_data->container->output_section;
        } else {
            _bfd_error_handler(_("%pB[%pA]: no output section for space %pA"),
                              obfd, osection, dst_data->container);
            return false;
        }
    }
...
static bool som_bfd_copy_private_bfd_data(bfd *ibfd, bfd *obfd)
{
    if (!ibfd || !obfd || !ibfd->xvec || !obfd->xvec)
        return false;
    ...
    struct som_exec_data *src = obj_som_exec_data(ibfd);
    struct som_exec_data *dst = bfd_zalloc(obfd, (bfd_size_type)sizeof(struct som_exec_data));

    if (!src || !dst)
        return false;

    obj_som_exec_data(obfd) = dst;
    memcpy(dst, src, sizeof(struct som_exec_data));
...
static bool som_bfd_print_private_bfd_data(bfd *abfd, void *farg)
{
    ...
    if (fprintf(f, "  type               %#x\n", auxhdr->type) < 0)
        return false;
    if (fprintf(f, "  length             %#x\n", auxhdr->length) < 0)
        return false;
    if (fprintf(f, "  text size          %#lx\n", (long)exec_header->exec_tsize) < 0)
        return false;
    if (fprintf(f, "  text memory offset %#lx\n", (long)exec_header->exec_tmem) < 0)
        return false;
    if (fprintf(f, "  text file offset   %#lx\n", (long)exec_header->exec_tfile) < 0)
        return false;
    if (fprintf(f, "  data size          %#lx\n", (long)exec_header->exec_dsize) < 0)
        return false;
    if (fprintf(f, "  data memory offset %#lx\n", (long)exec_header->exec_dmem) < 0)
        return false;
    if (fprintf(f, "  data file offset   %#lx\n", (long)exec_header->exec_dfile) < 0)
        return false;
    if (fprintf(f, "  bss size           %#lx\n", (long)exec_header->exec_bsize) < 0)
        return false;
    if (fprintf(f, "  entry point        %#lx\n", (long)exec_header->exec_entry) < 0)
        return false;
    if (fprintf(f, "  loader flags       %#lx\n", (long)exec_header->exec_flags) < 0)
        return false;
    if (fprintf(f, "  bss initializer    %#lx\n", (long)exec_header->exec_bfill) < 0)
        return false;
...
bool bfd_som_set_section_attributes(asection *section, int defined, int is_private, unsigned int sort_key, int spnum)
{
  struct som_section_data_struct *sec_data;
  struct som_copyable_section_data_struct *copy_data;

  if (!section)
    return false;

  sec_data = som_section_data(section);
  if (!sec_data)
    return false;

  copy_data = sec_data->copy_data;
  if (copy_data == NULL)
  {
    copy_data = bfd_zalloc(section->owner, sizeof(struct som_copyable_section_data_struct));
    if (!copy_data)
      return false;
    sec_data->copy_data = copy_data;
  }

  copy_data->sort_key = sort_key;
  copy_data->is_defined = defined;
  copy_data->is_private = is_private;
...
bool bfd_som_set_subsection_attributes(asection *section,
                                       asection *container,
                                       int access_ctr,
...
void bfd_som_set_symbol_type(asymbol *symbol, unsigned int type)
{
    ...
    som_symbol_data_t *data = som_symbol_data(symbol);
    if (data == NULL) {
        return;
    }
...
bool bfd_som_attach_aux_hdr(bfd *abfd, int type, const char *string)
{
    size_t len, pad, amt;
    void *hdr = NULL;
    ...
    if (type == VERSION_AUX_ID) {
        ...
        ((struct som_string_auxhdr *)hdr)->header_id.type = VERSION_AUX_ID;
        ((struct som_string_auxhdr *)hdr)->header_id.length = 4 + len + pad;
        ((struct som_string_auxhdr *)hdr)->string_length = len;
        memcpy(((struct som_string_auxhdr *)hdr)->string, string, len);
        if (pad > 0)
            memset(((struct som_string_auxhdr *)hdr)->string + len, 0, pad);
    } else if (type == COPYRIGHT_AUX_ID) {
        hdr = bfd_zalloc(abfd, amt);
        if (!hdr)
            return false;
        obj_som_copyright_hdr(abfd) = hdr;
        ((struct som_string_auxhdr *)hdr)->header_id.type = COPYRIGHT_AUX_ID;
        ((struct som_string_auxhdr *)hdr)->header_id.length = 4 + len + pad;
        ((struct som_string_auxhdr *)hdr)->string_length = len;
        memcpy(((struct som_string_auxhdr *)hdr)->string, string, len);
        if (pad > 0)
...
bool bfd_som_attach_compilation_unit(bfd *abfd,
                                     const char *name,
                                     const char *language_name,
                                     const char *product_id,
                                     const char *version_id)
{
    struct som_compilation_unit *n = bfd_zalloc(abfd, (bfd_size_type)sizeof(*n));
    if (!n)
        return false;
    ...
    for (int i = 0; i < 4; ++i)
    {
        if (fields[i])
        {
            ...
            *targets[i] = bfd_alloc(abfd, (bfd_size_type)len);
            if (!*targets[i])
                return false;
            memcpy(*targets[i], fields[i], len);
        }
    }
...
static bool som_get_section_contents(bfd *abfd, sec_ptr section, void *location, file_ptr offset, bfd_size_type count) {
    ...
    if ((section->flags & SEC_HAS_CONTENTS) == 0)
        return true;
    if ((offset > section->size) || (count > section->size - offset))
        return false;
...
static bool som_set_section_contents(bfd *abfd, sec_ptr section, const void *location, file_ptr offset, bfd_size_type count) {
    ...
    const subspace_dict_type *subspace_dict = som_section_data(section)->subspace_dict;
    if (!subspace_dict) {
        return false;
    }

    file_ptr file_offset = offset + subspace_dict->file_loc_init_value;
...
static bool som_set_arch_mach(bfd *abfd, enum bfd_architecture arch, unsigned long machine)
{
...
static char som_section_type(const char *s)
{
    ...
    for (const struct section_to_type *t = stt; t->section != NULL; t++) {
...
static int som_decode_symclass(asymbol *symbol) {
    ...
    char c;
    struct som_symbol *data = som_symbol_data(symbol);
    if (bfd_is_abs_section(symbol->section) ||
        (data && data->som_type == SYMBOL_TYPE_ABSOLUTE)) {
        c = 'a';
    } else {
...
static void som_get_symbol_info(bfd *ignore_abfd ATTRIBUTE_UNUSED, asymbol *symbol, symbol_info *ret) {
    ...
    ret->value = (ret->type != 'U' && symbol->section) ? symbol->value + symbol->section->vma : 0;
...
static bool som_bfd_count_ar_symbols(bfd *abfd, struct som_lst_header *lst_header, symindex *count)
{
    ...
    lst_filepos = bfd_tell(abfd);
    if (lst_filepos < 0) {
        bfd_set_error(bfd_error_file_truncated);
        return false;
    }
    if ((size_t)lst_filepos < sizeof(struct som_external_lst_header)) {
        bfd_set_error(bfd_error_file_truncated);
        return false;
    }
    lst_filepos -= sizeof(struct som_external_lst_header);
    ...
    if (!hash_table && lst_header->hash_size != 0)
        goto error_return;
...
static bool som_bfd_fill_in_ar_symbols(bfd *abfd, struct som_lst_header *lst_header, carsym **syms) {
    ...
    if (!abfd || !lst_header || !syms || !syms[0])
        return false;

    set = syms[0];
    lst_filepos = bfd_tell(abfd);
    if (lst_filepos < (file_ptr)sizeof(struct som_external_lst_header)) {
        bfd_set_error(bfd_error_bad_value);
        return false;
    }
    lst_filepos -= sizeof(struct som_external_lst_header);
...
static bool som_slurp_armap(bfd *abfd) {
    ...
    errno = 0;
    parsed_size = (unsigned int)strtoul(ar_header.ar_size, NULL, 10);
    if (errno != 0) {
        bfd_set_error(bfd_error_malformed_archive);
        return false;
    }
...
static bool som_bfd_prep_for_ar_write(bfd *abfd, unsigned int *num_syms, unsigned int *stringsize)
{
  if (!abfd || !num_syms || !stringsize)
    return false;
...
static unsigned int som_bfd_ar_symbol_hash(asymbol *symbol) {
    ...
    if (len == 1) {
        unsigned char c = (unsigned char)symbol->name[0];
        return 0x1000100 | (c << 16) | c;
    }
    ...
    unsigned char c1 = (unsigned char)symbol->name[1];
    unsigned char c2 = (unsigned char)symbol->name[len - 2];
    unsigned char c3 = (unsigned char)symbol->name[len - 1];

    return ((unsigned int)(len & 0x7f) << 24) | (c1 << 16) | (c2 << 8) | c3;
}

/* Do the bulk of the work required to write the SOM library
   symbol table.  */

static bool som_bfd_ar_write_symbol_stuff(bfd *abfd,
                                          unsigned int nsyms,
                                          ...
                                          unsigned elength)
{
    ...
    while (curr_bfd) {
        ...
        for (i = 0; i < curr_count; ++i, ++sym) {
            ...
            bfd_putb32((bfd_vma)(p - strings + 4), curr_lst_sym->name);
            ...
            symbol_pos = (unsigned int)((curr_lst_sym - lst_syms) * sizeof(struct som_external_lst_symbol_record)
                        + hash_size * 4
            ...
            slen = (unsigned int)strlen(sym->symbol.name);
...
static bool som_write_armap(bfd *abfd, unsigned int elength, struct orl *map ATTRIBUTE_UNUSED, unsigned int orl_count ATTRIBUTE_UNUSED, int stridx ATTRIBUTE_UNUSED)
{
    bfd *curr_bfd;
    struct stat statbuf;
    ...
    p = (unsigned char *)&lst;
    csum = 0;
    for (i = 0; i + 4 <= sizeof(struct som_external_lst_header) - sizeof(int); i += 4)
        csum ^= bfd_getb32(&p[i]);
...
static bool som_bfd_free_cached_info(bfd *abfd)
{
    ...
    if (format == bfd_object || format == bfd_core)
    {
        ...
        void *symtab = obj_som_symtab(abfd);
        if (symtab)
        {
            free(symtab);
            obj_som_symtab(abfd) = NULL;
        ...
        void *stringtab = obj_som_stringtab(abfd);
        if (stringtab)
        {
            free(stringtab);
            obj_som_stringtab(abfd) = NULL;
        ...
        for (; o != NULL; o = o->next)
        {
            ...
            if (som_section_data(o) && som_section_data(o)->reloc_stream)
            {
